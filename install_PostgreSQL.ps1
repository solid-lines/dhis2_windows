...
function Install-PostgreSQL {
	...
	Invoke-WebRequest -Uri ${downloadLink} -OutFile ${pg_download_file}
	
	# Install POstgreSQL, Postgis and PGAdmin. Create PostgreSQL service
	Write-Host "Installing PostgreSQL v${pg_version}..."
	Start-Process ${pg_download_file} -ArgumentList "--mode unattended", "--unattendedmodeui none", "--superpassword `"${pg_password}`"", "--serverport ${pg_port}", "--servicename `"${pg_service_name}`"", "--superaccount `"${pg_username}`"" -Wait;
	#Remove-Item -Path ${pg_download_file}
}
...
function Install-Postgis {	
    ...
    $postgis_url = "https://download.osgeo.org/postgis/windows/pg${pg_version}/postgis-bundle-pg${pg_version}x64-setup-${postgis_version}-1.exe"
    $postgis_file = ".\pg_${$pg_version}_postgis.exe"

    Invoke-WebRequest -Uri $postgis_url -OutFile $postgis_file

    Start-Process -FilePath $ostgis_file -ArgumentList "/S" -Wait
}
...
function Configure-PostgreSQL {
	...
	# Write-Host "pg_memory_gb = ${pg_memory_gb}"
	# Write-Host "pg_max_connections = ${pg_max_connections}"
...
if (-not (Check-PostgreSQL)) {
	...
	do {
		$user_response = Read-Host "Do you want to continue with the installation? (Y/N)"
		
		if ($user_response -match "^[Yy]$") {
			break
		} elseif ($user_response -match "^[Nn]$") {
			Write-Host "Exiting..."
			Exit 1
		} else {
