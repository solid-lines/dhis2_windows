...
function Download-Install-Nginx {
	...
	$nginx_url = "https://nginx.org/download/nginx-${proxy_version}.zip"
	$nginx_zip = ".\nginx.zip"
	...
	Expand-Archive -Path ${nginx_zip} -DestinationPath ${nginx_install_path} -Force
	#Remove-Item -Path $nginx_zip
}
